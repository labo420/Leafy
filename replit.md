...
- **LevelMilestoneBar** (inline in `app/(tabs)/index.tsx`): barra di progressione SVG a **trapezi colorati** — 5 badge (cerchi) con base allineata (tutti i bordi inferiori alla stessa quota = BASELINE_Y), crescenti in dimensione da sinistra (Germoglio 32px) a destra (Foresta 54px). Tra ogni coppia di badge: un trapezio SVG (`Path`) colorato con i colori "frutta" dell'esperto: Verde Kiwi (#8BC34A) → Giallo Limone (#F4D03F) → Arancione Mango (#FF8C42) → Rosso Mela (#E74C3C). Trapezi grigio-trasparenti di sfondo per i segmenti non raggiunti. Riempimento parziale proporzionale per il segmento in transizione. Badge badge crescenti di dimensione con bordo colorato se raggiunti.
