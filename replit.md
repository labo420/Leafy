...
- **LevelMilestoneBar** (inline in `app/(tabs)/index.tsx`): barra di progressione **ascendente** SVG — 5 nodi sfalsati in altezza (effetto "salita" da Germoglio→Foresta) collegati da linee SVG spesse (`strokeWidth: 8`, `strokeLinecap: round`) con colori "frutta" per segmento: Verde Kiwi (#8BC34A) → Giallo Limone (#F4D03F) → Arancione Mango (#FF8C42) → Rosso Mela (#E74C3C). Segmenti non raggiunti in grigio trasparente. Badge crescenti di dimensione (32→54px). Nodi raggiunti con bordo colorato + sfondo semi-trasparente.
- **Bottom tab bar** (`app/(tabs)/_layout.tsx`): redesign estetico — indicatore attivo con dot verde sotto l'icona + label animata (appare solo per il tab attivo con fade+slide), pill semi-trasparente con scale animato via `useSharedValue`+`interpolate(Extrapolation.CLAMP)`. Scan button floating con anello e glow animati. Bordo top arrotondato 28px, ombra verde più profonda. Labels nascoste con `tabBarShowLabel: false` (gestite custom). Colori espliciti: attivo `#2E7D32`, inattivo `#8EA99A`.
